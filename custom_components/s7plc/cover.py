...
class S7Cover(S7BaseEntity, CoverEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        # Two independent feedback sources can each decide movement is over
        # in the same update (end-stops via use_state_topics, and real
        # movement feedback via cover_status_address/the boolean opening/
        # closing/stopped addresses below). Collect into local flags and
        # dispatch once per direction, so an update where both agree can't
        # schedule _complete_operation twice for the same movement.
        stop_open = False
        stop_close = False
