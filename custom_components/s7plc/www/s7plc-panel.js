...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  const movementFeedback=entity.cover_status_address&&!(isTraditionalLike&&positionFeedback==="status")?"status":isTraditionalLike&&positionFeedback!=="status"&&(entity.cover_opening_address||entity.cover_closing_address||entity.cover_stopped_address)?"bits":"none";
...
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);const isTraditionalLike=ui.cover_control_mode==="traditional"||ui.cover_control_mode==="toggle";if(ui.preserve_legacy&&isTraditionalLike){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;return entity;}if(isTraditionalLike){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);const mode=ui.cover_position_feedback;entity.cover_position_feedback=mode;if(mode==="status"){delete entity.use_state_topics;delete entity.opening_state_address;delete entity.closing_state_address;["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}else{if(ui.cover_movement_feedback!=="status")["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);if(mode==="timed"){entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;}else{entity.use_state_topics=true;if(mode==="opening")delete entity.closing_state_address;if(mode==="closing")delete entity.opening_state_address;}}}else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;}
  const traditionalStatus=isTraditionalLike&&ui.cover_position_feedback==="status";if(ui.cover_movement_feedback==="none"&&!traditionalStatus){["cover_opening_address","cover_closing_address","cover_stopped_address","cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);}else if(ui.cover_movement_feedback==="bits"){["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);}else {["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}
