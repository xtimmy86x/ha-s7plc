...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  // not be assumed to also cover the other, but a shared
  // cover_status_address still defaults to serving movement too (matches
  // the pre-selector legacy behavior) unless position_feedback already
  // claims it AND there's a genuine alternative (movement bits) to fall
  // back to instead. Plain traditional covers keep the original coupled
  // inference (bits are only ever considered when position feedback isn't
  // already "status") since their runtime doesn't implement the
