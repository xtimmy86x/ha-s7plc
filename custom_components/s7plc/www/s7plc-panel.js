...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  // Position and movement feedback are independently selectable - a status
  // word chosen for one purpose must not be assumed to also cover the
  // other, but a shared cover_status_address still defaults to serving
  // movement too (matches the pre-selector legacy behavior) unless
  // position_feedback already claims it AND there's a genuine alternative
  // (movement bits) to fall back to instead.
  const movementBits=isTraditionalLike&&(entity.cover_opening_address||entity.cover_closing_address||entity.cover_stopped_address);
  const movementFeedback=entity.cover_status_address&&!(positionFeedback==="status"&&movementBits)?"status":movementBits?"bits":"none";
...
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);const isTraditionalLike=ui.cover_control_mode==="traditional"||ui.cover_control_mode==="toggle";if(ui.preserve_legacy&&isTraditionalLike){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;return entity;}if(isTraditionalLike){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);const mode=ui.cover_position_feedback;entity.cover_position_feedback=mode;if(mode==="timed"){entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;}else if(mode==="status"){delete entity.use_state_topics;delete entity.opening_state_address;delete entity.closing_state_address;}else{entity.use_state_topics=true;if(mode==="opening")delete entity.closing_state_address;if(mode==="closing")delete entity.opening_state_address;}}else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;}
  const positionStatus=isTraditionalLike&&ui.cover_position_feedback==="status",movementStatus=ui.cover_movement_feedback==="status";if(!positionStatus)COVER_STATUS_POSITION_VALUE_FIELDS.forEach(key=>delete entity[key]);if(!movementStatus)COVER_STATUS_MOVEMENT_VALUE_FIELDS.forEach(key=>delete entity[key]);if(!positionStatus&&!movementStatus)delete entity.cover_status_address;if(!(isTraditionalLike&&ui.cover_movement_feedback==="bits"))["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);
