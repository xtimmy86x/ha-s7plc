...
const TRANSLATIONS = {
  en: {
    loading:"Loading configuration…", no_plc:"No PLC configured", no_plc_help:"First add the Siemens S7 integration from Devices & services.", title:"S7 PLC configuration", subtitle:"Configure entities visually or use YAML in the advanced options.", entities:"entities", connected:"Connected", disconnected:"Disconnected", reload_help:"Changes are applied by automatically reloading the integration.", add:"Add", empty:"No entities", empty_help:"Use “Add” to create the first one.", entity:"Entity", edit:"Edit", delete:"Delete", enabled_help:"Enable this option", required:"Required", address_example:"e.g. DB1,REAL0", name_example:"e.g. Living room temperature", address_help:"Siemens S7 format, for example DB1,REAL0", plc_connection:"PLC connection", plc_connection_help:"Data addresses and sources", behavior:"Behavior", behavior_help:"Options specific to this entity", ha_details:"Details in Home Assistant", ha_details_help:"Name, area, and update frequency", new_entity:"New entity", edit_entity:"Edit entity", configure_new:"Configure a new entity", update_configuration:"Update the configuration", save_help:"Fields are saved directly in the PLC configuration.", editor_mode:"Editor mode", visual_editor:"Visual editor", guided:"Guided and simple", advanced:"Advanced control", yaml_warning:"Advanced mode lets you manually edit every field. Use a YAML mapping for a single entity.", yaml_label:"YAML configuration", cancel:"Cancel", save:"Save changes", required_error:"Fill in all required fields.", cover_required_error:"Enter the required addresses for the selected cover type.", climate_required_error:"Enter the target temperature address.", delete_confirm:"Delete this entity? This action cannot be undone.",
...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  set hass(value) { const previous=this.language; this._hass = value; if (!this._loaded) this.load(); else if(previous!==this.language)this.render(); }
  ...
  entityCards(entry){const type=this.type||TYPES[0],items=entry.entities[type];if(!items.length)return `<div class="empty small"><ha-icon icon="mdi:playlist-plus"></ha-icon><h3>${this.t('empty')}</h3><p>${this.t('empty_help')}</p></div>`;return items.map((item,i)=>`<article><div class="entity-icon"><ha-icon icon="mdi:${this.icon(type)}"></ha-icon></div><div class="details"><b>${this.escape(item.name||item.address||item.state_address||item.current_temperature_address||`${this.t('entity')} ${i+1}`)}</b><code>${this.escape(item.address||item.state_address||item.current_temperature_address||item.source_entity||'—')}</code><div>${Object.entries(item).filter(([k])=>k!=='name').slice(0,5).map(([k,v])=>`<span>${this.escape(this.t(`fields.${k}`))}: ${this.escape(String(v))}</span>`).join('')}</div></div><button data-edit="${i}" class="icon-btn" title="${this.t('edit')}"><ha-icon icon="mdi:pencil"></ha-icon></button><button data-delete="${i}" class="icon-btn danger" title="${this.t('delete')}"><ha-icon icon="mdi:delete"></ha-icon></button></article>`).join('');}
  ...
  async remove(index){if(!confirm(this.t('delete_confirm')))return;await this._hass.callWS({type:"s7plc/config/delete_entity",entry_id:this.entryId,entity_type:this.type||TYPES[0],index});this._loaded=false;await this.load();}
