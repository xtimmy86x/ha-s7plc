...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  openEditor(index=null){const entry=this.entries.find(e=>e.entry_id===this.entryId),type=this.type||TYPES[0],raw=index===null?{}:entry.entities[type][index],initial=this.inferred(raw,type),dialog=document.createElement('ha-dialog');dialog.open=true;dialog.headerTitle=index===null?this.t('editor.new_entity'):this.t('editor.edit_entity');dialog.style.setProperty('--mdc-dialog-max-width','min(940px,95vw)');dialog.style.setProperty('--mdc-dialog-min-width','min(940px,95vw)');dialog.style.setProperty('--dialog-content-padding','0');dialog.innerHTML=`<style>${this.dialogStyles}</style><div class="dialog-body"><div class="editor-intro"><span class="editor-type-icon"><ha-icon icon="mdi:${this.icon(type)}"></ha-icon></span><div><span class="eyebrow">${this.t(`entity_types.${type}.label`)}</span><h3>${index===null?this.t('editor.configure_new'):this.t('editor.update_configuration')}</h3><p>${this.t('editor.save_help')}</p></div></div><div class="mode-tabs" role="tablist" aria-label="${this.t('editor.editor_mode')}"><button class="active" data-mode="visual" role="tab"><ha-icon icon="mdi:form-select"></ha-icon><span>${this.t('editor.visual_editor')}<small>${this.t('editor.guided')}</small></span></button><button data-mode="yaml" role="tab"><ha-icon icon="mdi:code-braces"></ha-icon><span>YAML<small>${this.t('editor.advanced')}</small></span></button></div><form class="visual-form">${this.editorSections(type,initial)}</form><div class="yaml-editor" style="display:none"><ha-alert alert-type="warning">${this.t('editor.yaml_warning')}</ha-alert><textarea spellcheck="false" aria-label="${this.t('editor.yaml_label')}">${this.escape(this.toYaml(raw))}</textarea></div><ha-alert class="editor-error" alert-type="error" style="display:none"></ha-alert></div><ha-dialog-footer slot="footer"><ha-button slot="secondaryAction" appearance="plain">${this.t('actions.cancel')}</ha-button><ha-button slot="primaryAction" appearance="accent">${this.t('actions.save')}</ha-button></ha-dialog-footer>`;document.body.appendChild(dialog);
    ...
    const syncMode=()=>{if(type==='covers'){const control=form.elements.cover_control_mode.value,positionFeedback=form.elements.cover_position_feedback.value,movement=form.elements.cover_movement_feedback.value,stop=form.elements.cover_stop_enabled.value==='enabled',tilt=form.elements.cover_tilt_enabled.value==='enabled';const visible=new Set(control==='position'?['position_state_address','position_command_address','invert_position']:['open_command_address','close_command_address','operate_time']);if(control==='traditional'&&positionFeedback==='endstops'){visible.add('opening_state_address');visible.add('closing_state_address');}if(movement==='bits'&&control==='traditional')['cover_opening_address','cover_closing_address','cover_stopped_address'].forEach(k=>visible.add(k));if(movement==='status')['cover_status_address',...COVER_STATUS_VALUE_FIELDS].forEach(k=>visible.add(k));if(control==='position'&&stop)['stop_command_address','stop_pulse_duration'].forEach(k=>visible.add(k));if(control==='position'&&tilt)['tilt_state_address','tilt_command_address','invert_tilt'].forEach(k=>visible.add(k));const governed=[...COVER_TRADITIONAL_FIELDS,...COVER_POSITION_FIELDS,'cover_status_address',...COVER_STATUS_VALUE_FIELDS,'stop_command_address','stop_pulse_duration'];governed.forEach(k=>form.querySelector(`[data-field="${k}"]`)?.classList.toggle('hidden-field',!visible.has(k)));form.querySelector('[data-section="cover-position-feedback"]').classList.toggle('hidden-field',control==='position');form.querySelector('[data-section="cover-stop"]').classList.toggle('hidden-field',control!=='position');form.querySelector('[data-section="cover-tilt"]').classList.toggle('hidden-field',control!=='position');const bits=form.querySelector('input[name="cover_movement_feedback"][value="bits"]');bits.closest('.control-card').classList.toggle('hidden-field',control==='position');if(control==='position'&&bits.checked)form.querySelector('input[name="cover_movement_feedback"][value="none"]').checked=true;return;}if(type==='climates'){const value=name=>form.querySelector(`input[name="${name}"]:checked`)?.value,control=value('control_mode'),directFunction=value('climate_direct_function'),directFeedback=value('climate_direct_feedback'),modeControl=value('climate_mode_control'),actionFeedback=value('climate_action_feedback'),coded=modeControl?.includes('coded'),onOff=modeControl?.includes('on_off');const visible=new Set(['control_mode','current_temperature_address','min_temp','max_temp','temp_step','name','area','scan_interval','climate_action_feedback']);if(control==='direct'){['climate_direct_function','climate_direct_feedback'].forEach(k=>visible.add(k));if(directFunction!=='cool')visible.add('heating_output_address');if(directFunction!=='heat')visible.add('cooling_output_address');if(directFeedback==='plc')['heating_action_address','cooling_action_address'].forEach(k=>visible.add(k));}else{['target_temperature_address','climate_mode_control',...CLIMATE_PRESET_VALUE_FIELDS].forEach(k=>visible.add(k));if(coded)['preset_mode_address','preset_mode_bidirectional'].forEach(k=>visible.add(k));if(onOff)visible.add('on_off_address');}if(actionFeedback==='plc')['hvac_status_address',...CLIMATE_STATUS_VALUE_FIELDS].forEach(k=>visible.add(k));form.querySelectorAll('[data-field]').forEach(field=>field.classList.toggle('hidden-field',!visible.has(field.dataset.field)));['direct-function','direct-feedback'].forEach(k=>form.querySelector(`[data-section="climate-${k}"]`).classList.toggle('hidden-field',control!=='direct'));['mode-control','modes','mode-feedback'].forEach(k=>form.querySelector(`[data-section="climate-${k}"]`).classList.toggle('hidden-field',control!=='setpoint'||(k==='mode-feedback'&&!coded)));return;}const sel=form.elements.control_mode;if(!sel)return;const hidden=MODE_HIDDEN[type]?.[sel.value]||[];form.querySelectorAll('[data-field]').forEach(l=>l.classList.toggle('hidden-field',hidden.includes(l.dataset.field)));};
