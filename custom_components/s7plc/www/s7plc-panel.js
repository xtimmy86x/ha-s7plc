...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  const legacyPositionFeedback=entity.cover_status_address?"status":entity.opening_state_address&&entity.closing_state_address?"both":entity.opening_state_address?"opening":entity.closing_state_address?"closing":"timed";
  const positionFeedback=control==="traditional"?(entity.cover_position_feedback||legacyPositionFeedback):"timed";
  const movementFeedback=control==="position"&&entity.cover_status_address?"status":control==="traditional"&&positionFeedback!=="status"&&(entity.cover_opening_address||entity.cover_closing_address||entity.cover_stopped_address)?"bits":"none";
...
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);if(ui.cover_control_mode==="traditional"){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);const mode=ui.cover_position_feedback;entity.cover_position_feedback=mode;if(mode==="status"){delete entity.use_state_topics;delete entity.opening_state_address;delete entity.closing_state_address;delete entity.operate_time;["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}else{["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);if(mode==="timed"){entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;}else{entity.use_state_topics=true;if(mode==="opening")delete entity.closing_state_address;if(mode==="closing")delete entity.opening_state_address;delete entity.operate_time;}}}else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;}
