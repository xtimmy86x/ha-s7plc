    ...
    async def _update_outputs(self) -> None:
        ...
        if self._hvac_mode == HVACMode.OFF:
            ...
            if self._heating_output_address:
                await self._async_write(self._heating_output_address, False)
            if self._cooling_output_address:
                await self._async_write(self._cooling_output_address, False)
            return
        ...
        if self._heating_output_address:
            if self._hvac_mode in (HVACMode.HEAT, HVACMode.HEAT_COOL):
                await self._async_write(self._heating_output_address, heating_needed)
            else:
                await self._async_write(self._heating_output_address, False)
        ...
        if self._cooling_output_address:
            if self._hvac_mode in (HVACMode.COOL, HVACMode.HEAT_COOL):
                await self._async_write(self._cooling_output_address, cooling_needed)
            else:
                await self._async_write(self._cooling_output_address, False)
...
class S7ClimateSetpointControl(
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        await self._async_write(self._target_temp_address, float(temperature))
